    ...
    HistoricalAction.find({ userId: req.user._id }, (err, actions) => {
      if (err) res.status(500).send(err);
          ...
          switch (req.query.sort) {
            case "date":
              ...
              for (action of response.actions) {
                if (newActions[moment(action.endDate).format("Y-MM-DD")])
                  newActions[moment(action.endDate).format("Y-MM-DD")].push(
                    action
                  );
                else
                  newActions[moment(action.endDate).format("Y-MM-DD")] = [
                    action
                  ];
              }
        ...
        if (response.actions.length > 50) {
          ...
          response.page =
            req.query.page && req.query.page <= response.pages
...
history
  .route("/:id")
  .get((req, res) => {
    HistoricalAction.findOne(
      { _id: req.params.id, userId: req.user._id },
      (err, action) => {
        if (err || !action) return res.status(404).send(err);
        return res.send(action);
      }
    );
  })
  .delete((req, res) => {
    HistoricalAction.findOneAndRemove(
      { _id: req.params.id, userId: req.user._id },
      err => {
